...
class SwapDisassembler(Disassembler):
    ...
    def get_amount_to_redeem(self, token_in_address: Address, fraction: float | Decimal) -> int:
        if token_in_address == "0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE":
            ...
            return int(
                Decimal(token_in_contract.functions.balanceOf(self.avatar_safe_address).call()) * Decimal(fraction))
        ...
        for element in exit_arguments:
            ...
            if token_in == "0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE":
                ...
                approve_curve = ApproveCurve(
                    blockchain=self.blockchain, token_address=token_in, spender=swap_pool.address,
