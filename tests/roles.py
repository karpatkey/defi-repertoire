...
def deploy_roles(w3: Web3, avatar):
    ...
    bytecode_without_default_constructor = roles_bytecode[
        : -len(role_constructor_bytes)
    ]

    ctract = w3.eth.contract(
        abi=roles_abi, bytecode=bytecode_without_default_constructor
    )

    owner = avatar = target = to_checksum_address(avatar)
    tx_hash = ctract.constructor(owner, avatar, target).transact(
        {"from": avatar}
    )  # deploy!
    roles_ctract_address = w3.eth.wait_for_transaction_receipt(tx_hash, timeout=5)[
        "contractAddress"
    ]
...
def setup_common_roles(safe: SimpleSafe, roles_ctract):
    ...
    enable_module_roles = safe.contract.functions.enableModule(
        roles_ctract.address
    ).build_transaction({"from": safe.address})["data"]
    ...
    for role_number in range(1, 6):
        ...
        enable_module = roles_ctract.functions.enableModule(
            account.address
        ).build_transaction({"from": safe.address})["data"]
        assign_role = roles_ctract.functions.assignRoles(
            account.address, [role_number], [True]
        ).build_transaction({"from": safe.address})["data"]
        txns.extend(
            [
